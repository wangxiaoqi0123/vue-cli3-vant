...
// 加载水印
export default {
  goWatermark(username) {
    if (username) {
      // let date = this.dateformat(new Date(),"yyyy-MM-dd hh:mm:ss");
  ...
  watermark(watermark_text) {
    var defaultSettings = {
      ...
      watermark_y_space: 25, // 水印y轴间隔
      watermark_font: "微软雅黑", // 水印字体
      watermark_color: "#c8c9cc", // 水印字体颜色
    ...
    if (!otdiv) {
      ...
      otdiv.style.top = 0;
      otdiv.style.zIndex = "999999999999999";
  ...
  remove() {
    let otdivid = document.getElementById("otdivid");
    if (otdivid) {
      document.body.removeChild(otdivid);
    }
  }
};
